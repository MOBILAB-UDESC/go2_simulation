...
namespace utils{
	...
	{
	std::lock_guard<std::mutex> lock(this->pose_mutex);
	return math::quatToRPY(this->quaternion);
	}
	...
	{
	std::lock_guard<std::mutex> lock(this->pose_mutex);
	return math::quatToRotMat(this->quaternion);
	}
	...
	Pose::transformation_matrix_t Pose::toTransformationMatrix() const
	{
		...
		{
			std::lock_guard<std::mutex> lock(this->pose_mutex);
			T.block<3,3>(0,0) = math::quatToRotMat(this->quaternion);//this->quaternion.toRotationMatrix();
